        ...
        switch (riskLevel) {
            ...
            case 'moderate':
                return {
                    ...
                    badgeText: 'var(--pastel-peach-text)',
                    dotBg: 'var(--pastel-peach-text)'
                };
            default:
                return {
                    color: '#16a34a',
                    fillColor: '#22c55e',
                    badgeBg: 'var(--pastel-mint-bg)',
                    badgeText: 'var(--pastel-mint-text)',
                    dotBg: 'var(--pastel-mint-text)'
                };
        }
    };
    ...
    return (
        <div className="w-full flex flex-col gap-6 text-left relative max-w-7xl mx-auto pb-12 animate-fade-in-up">
            ...
            >
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex flex-wrap items-center gap-2.5">
                            ...
                            <span
                                className="px-3 py-1 rounded-full text-[11px] font-black uppercase tracking-wider flex items-center gap-1.5 shadow-2xs"
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        onClick={loadAlerts}
                        disabled={loading}
                        className="px-4 py-2.5 rounded-full text-xs font-bold transition shadow-xs cursor-pointer flex items-center gap-2"
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
                <span className="text-xs font-black uppercase tracking-wider flex items-center gap-1 shrink-0 pl-1" style={{ color: 'var(--text-secondary)' }}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-[620px]">
                {/* Map Area */}
                <div
                    className="lg:col-span-2 rounded-3xl p-2 shadow-xs flex flex-col relative overflow-hidden h-[540px] lg:h-[640px]"
                    style={{ background: 'var(--card-bg)', border: '1px solid var(--border)' }}
                >
                    {loading ? (
                        <div
                            className="w-full h-full flex items-center justify-center text-xs font-bold rounded-2xl"
                    ...
                    {/* Overlay Legend */}
                    <div
                        className="absolute bottom-6 left-6 z-[1000] backdrop-blur-md rounded-2xl p-3 shadow-md flex items-center gap-4 text-xs font-bold"
                    ...
                    >
                        ...
                        <div className="flex items-center gap-1.5" style={{ color: 'var(--pastel-peach-text)' }}>
                            <span className="w-3 h-3 rounded-full" style={{ background: 'var(--pastel-peach-text)' }} />
                            <span>Moderate</span>
                        </div>
                        <div className="flex items-center gap-1.5" style={{ color: 'var(--pastel-mint-text)' }}>
                            <span className="w-3 h-3 rounded-full" style={{ background: 'var(--pastel-mint-text)' }} />
                            <span>Watch</span>
                        </div>
                    </div>
                </div>
                ...
                <div
                    className="rounded-3xl p-6 shadow-xs flex flex-col gap-5 h-[540px] lg:h-[640px] overflow-hidden"
                    style={{ background: 'var(--card-bg)', border: '1px solid var(--border)' }}
                >
                    <div className="flex items-center justify-between pb-3 shrink-0" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                    ...
                    <div className="flex-1 overflow-y-auto pr-1 flex flex-col gap-3">
                        {filteredAlerts.length === 0 && !loading && (
                            <div className="text-center py-12 flex flex-col items-center justify-center gap-2" style={{ color: 'var(--text-secondary)' }}>
                                <CheckCircle2 className="w-8 h-8" style={{ color: 'var(--pastel-mint-text)' }} />
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center justify-between gap-2">
                                        <span className="font-black text-xs sm:text-sm" style={{ color: 'var(--text-primary)' }}>
                                            {a.diseaseCategory}
                                        </span>
                                        <span
                                            className="px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider border"
                                            style={{ background: style.badgeBg, color: style.badgeText, borderColor: style.badgeText }}
                                        >
                                            {a.riskLevel}
                                        </span>
                                    </div>
                    ...
                    {selectedAlert && (
                        <div
                            className="pt-4 flex flex-col gap-3 shrink-0 p-3 rounded-2xl"
                            style={{ borderTop: '1px solid var(--border-subtle)', background: 'var(--bg-surface)' }}
                        >
                            <div className="flex items-center justify-between">
                                <h4 className="font-black text-xs flex items-center gap-1.5" style={{ color: 'var(--text-primary)' }}>
                                    <BarChart3 className="w-3.5 h-3.5" style={{ color: 'var(--accent)' }} />
                                    <span>7-Day Trajectory: {selectedAlert.diseaseCategory}</span>
                                </h4>
                                <span className="text-[10px] font-bold" style={{ color: 'var(--text-secondary)' }}>{selectedAlert.geohash}</span>
                            </div>

                            {trendLoading ? (
                                <div className="h-24 flex items-center justify-center text-xs font-bold animate-pulse" style={{ color: 'var(--text-secondary)' }}>
                                    Loading trend data...
                                </div>
                            ) : trendData.length > 0 ? (
                                        ...
                                        return (
                                            ...
                                            >
                                                <div
                                                    ...
                                                    style={{ height: `${hPct}%`, background: 'var(--accent)' }}
                                                />
                                                <span className="text-[9px] mt-1 font-bold" style={{ color: 'var(--text-secondary)' }}>
                                                    {new Date(d.date).getDate()}
                                                </span>
                                                <div
                                                    className="absolute -top-7 text-[10px] px-2 py-0.5 rounded-lg hidden group-hover:block z-20 font-bold whitespace-nowrap shadow-md"
