...
import { RefreshCcw, Check, Clock, Stethoscope, FileText, XCircle } from 'lucide-react';
...
const ClinicReferrals = () => {
    ...
    const loadData = async () => {
        ...
        setError('');
        try {
            const res = await clinicService.getIncomingReferrals();
            setReferrals(res.referrals || []);
        } catch (err) {
            setError('Failed to load incoming referrals');
        } finally {
            setLoading(false);
        }
    };
    ...
    const updateStatus = async (id, status) => {
        setActionError('');
        try {
            await clinicService.updateIncomingReferral(id, { status });
            loadData();
        } catch (err) {
            setActionError(err.response?.data?.error || `Failed to update status to ${status}`);
        }
    };
    ...
    const handleRecordOutcome = async (e) => {
        ...
        setActionError('');
        try {
            await clinicService.updateIncomingReferral(selectedReferral.id, { 
            ...
            setActionError(err.response?.data?.error || 'Failed to record outcome');
        }
    };
                    ...
                    <tbody>
                        {referrals.map((item) => (
                            <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50">
                                ...
                                <td className="px-4 py-3 text-slate-600">{item.referringHealthWorker?.healthWorkerProfile?.name || 'Unknown HW'}</td>
